import { Zap } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { Button } from './ui/button';

export const LoginPage = () => {
  const { login } = useAuth();

  return (
    <div className="login-container grid-bg">
      <div className="login-card text-center">
        {/* Logo */}
        <div className="mb-8">
          <div className="w-16 h-16 mx-auto mb-6 rounded bg-gradient-to-br from-[#6441a5] to-[#B0E301] flex items-center justify-center">
            <Zap className="w-8 h-8 text-black" />
          </div>
          <h1 className="logo-pixel-large text-center">
            <span className="logo-fit">FIT</span>
            <span className="logo-quest">QUEST</span>
          </h1>
          <p className="text-[#A1A1AA] mt-4 text-sm">Level Up Your Life</p>
        </div>

        {/* Welcome Text */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold mb-2">Bienvenue</h2>
          <p className="text-[#52525B] text-sm">
            Connectez-vous pour accéder à votre coach personnel
          </p>
        </div>

        {/* Google Login Button */}
        <Button
          onClick={login}
          className="w-full py-4 text-base flex items-center justify-center gap-3"
          data-testid="google-login-btn"
          variant="brand"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="currentColor"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="currentColor"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="currentColor"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Continuer avec Google
        </Button>

        <p className="mt-8 text-xs text-[#52525B]">
          En vous connectant, vous acceptez nos conditions d'utilisation
        </p>
      </div>
    </div>
  );
};
